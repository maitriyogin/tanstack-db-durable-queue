...
} from '../db/shoppingListClient';
import { budgetsCollection } from '../db/budgetClient';
import type { Budget, ShoppingList } from '../db/types';
...
//
// All mutations go through `shoppingListsCollection` (durable-queue wrapped)
// so item adds/removes survive offline + drain on reconnect just like todos.
export function TodoShoppingPanel({ todoId }: { todoId: string }) {
  ...
  const { data: lists } = useLiveQuery((q) =>
    q.from({ list: shoppingListsCollection }),
  );
  const { data: budgets } = useLiveQuery((q) =>
    q.from({ b: budgetsCollection }),
  );

  // Find the list linked to this todo. Filtering client-side rather than
  // pushing predicate to the live query so the per-row component stays
  // simple; the lists collection is small.
  const list = ((lists as Array<ShoppingList> | undefined) ?? []).find(
    (l) => l.todoId === todoId,
  );
  const budget = list
    ? ((budgets as Array<Budget> | undefined) ?? []).find(
        (b) => b.shoppingListId === list.id,
      )
    : undefined;
